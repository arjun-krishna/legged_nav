...
from legged_gym.pointgoal_nets import resnet
...
def build_rnn_inputs(
    ...
    N = rnn_states.size(1)
    print("**debug = ", rnn_states.shape, rnn_states.size(1))
    print("**debug = ", x.shape, x.size(0))
    T = x.size(0) // N
    dones = torch.logical_not(not_dones)
    print("**debug = ", dones.shape)
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(
        ...
        if normalize_visual_inputs:
            self.running_mean_and_var = RunningMeanAndVar(1)
        else:
            self.running_mean_and_var = nn.Sequential()
        self.backbone = make_backbone(1, 32, 16)
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        # obs: (batch x channel x height x width)
        x = F.avg_pool2d(obs, 2)
        x = self.running_mean_and_var(x)
        x = self.backbone(x)
        x = self.compression(x)
        return x

class PointNavResNet(nn.Module):
    # hardcoded
    ...
    def __init__(
        ...
        rnn_input_size = 64 + hidden_size
        make_backbone = getattr(resnet, backbone)

        self.visual_encoder = ResNetEncoder(make_backbone=make_backbone)
        ...
        self.state_encoder = LSTMStateEncoder(rnn_input_size, hidden_size, num_layers)
        self.output_size = hidden_size
        self.eval()

    def forward(self, cam_obs, goal_obs, rnn_hidden_states, prev_actions, masks):
        x = []
        viz_feat = self.visual_encoder(cam_obs)
        viz_feat = self.visual_fc(viz_feat)
        x.append(viz_feat)
        x.append(self.tgt_embeding(goal_obs))

        prev_actions = prev_actions.squeeze(-1)
        start_token = torch.zeros_like(prev_actions)
        prev_actions = self.prev_action_embedding(torch.where(masks.view(-1), prev_actions + 1, start_token))
...
class Policy(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.net = PointNavResNet()
        self.action_distribution = CategoricalNet(
            self.net.output_size, 4
        )
        self.critic = CriticHead(self.net.output_size)
    
    def act(self, cam_obs, goal_obs, rnn_hidden_states, prev_actions, masks, deterministic=False):
        features, rnn_hidden_states = self.net(cam_obs, goal_obs, rnn_hidden_states, prev_actions, masks)
...
class ExportedPolicy(nn.Module):
    ...
    def act(self, cam_obs, goal_obs, rnn_hidden_states, prev_actions, masks, deterministic=False):
        return self.actor_critic.act(cam_obs, goal_obs, rnn_hidden_states, prev_actions, masks, deterministic)

"""
import torch
from legged_gym.pointgoal_nets.policy import ExportedPolicy
